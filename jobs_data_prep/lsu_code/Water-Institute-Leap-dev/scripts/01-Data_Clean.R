...
# ----- Data Load ----- 
raw$sector_alloc <- read_csv("data/raw/sector_allocation_prep.csv")
raw$energy <- read_csv("data/raw/baseline_costs_and_production_by_prodtype.csv")
raw$industrial <- read_csv("data/raw/industrial_energy_cost.csv")
raw$trans_elec <- read_csv("data/raw/transportation_electric_efficiency_cost.csv") %>%
  mutate(
...
raw$trans_heavy <- read_csv("data/raw/transportation_heavy_duty_fuel_switch_cost.csv") %>%
  rename(fuel_switch_net_cost_heavy = `fuel_switch_net_cost_$`)
raw$trans_non_elec <- read_csv("data/raw/transportation_non_electric_efficiency_cost.csv") %>%
  mutate(
...
raw$trans_rail <- read_csv("data/raw/transportation_rail_fuel_switch_cost.csv") %>%
  mutate(
...
raw$trans_light <- read_csv("data/raw/transportion_light_duty_fuel_switch_cost.csv") %>%
  rename(fuel_switch_net_cost_light = `fuel_switch_net_cost_$`)
raw$ccs <- read_csv("data/raw/fugitive_emissions_and_ccs.csv") %>%
...
# ----- Data Save -----
saveRDS(energy_shocks, "data/intermediate/energy_data.rds")
saveRDS(shocks, "data/intermediate/power_shocks.rds")
saveRDS(ind_shocks, "data/intermediate/ind_shocks.rds")
saveRDS(ccs_shocks, "data/intermediate/ccs_shocks.rds")